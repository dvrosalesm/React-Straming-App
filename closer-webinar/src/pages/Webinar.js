...
class Webinar extends Component {
    ...
    render() {
        ...
        if(this.state.loading) 
            return (
                <WebinarLayout companyLogo={this.state.event.company_logo} webinarBackground={this.state.event.background} hasLoaded={!this.state.loading}> 
        ...
        if(!this.state.assistance && !this.state.loading)
            return (
                <WebinarLayout companyLogo={this.state.event.company_logo} webinarBackground={this.state.event.background} hasLoaded={false} showHeader={false}> 
                    <FlexboxGrid justify="center">
                        <FlexboxGrid.Item componentClass={Col} colspan={24} md={16} sm={24}>
                            <Assistance onAssistance={this.onAssistance} eventId={this.props.match.params.closerId} eventLogo={this.state.event.logo}/>
                        </FlexboxGrid.Item>
                    </FlexboxGrid>
                </WebinarLayout>
            );
        ...
        return (
            <WebinarLayout companyLogo={this.state.event.company_logo} webinarBackground={this.state.event.background} hasLoaded={!this.state.loading} handleLogout={this.onLogout} showHeader={true}>
                <FlexboxGrid>
                ...
                : 
                    <FlexboxGrid>
                        ...
                        { this.state.sizes.chat !== 0 ?
                            <FlexboxGrid.Item componentClass={Col} colspan={this.state.sizes.chat} md={this.state.sizes.chat} xs={24}>
                ...
                <FlexboxGrid>
                    { this.state.sizes.documents !== 0 ?
                        <FlexboxGrid.Item componentClass={Col} colspan={this.state.sizes.documents} md={this.state.sizes.documents} xs={24}>
                            <Documents 
                                documents={this.state.event.documents}
                            />
                        </FlexboxGrid.Item>
                    : null
                    }
                    { this.state.sizes.notes !== 0 && !this.state.sizes.special_case?
                        <FlexboxGrid.Item componentClass={Col} colspan={this.state.sizes.notes} md={this.state.sizes.notes} xs={24}>
                            <Notes eventId={this.props.match.params.closerId} />
                        </FlexboxGrid.Item>
                    : null
                    }
